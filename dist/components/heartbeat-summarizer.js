...
// 報告輸出路徑
const getReportPath = (workspaceDir) => join(workspaceDir, 'reports', 'daily_report_latest.md');
// Memory 日誌路徑
const getMemoryDir = (workspaceDir) => join(workspaceDir, 'memory');
/**
 * 獲取最新的 session log 文件
 */
function getLatestSessionLog(memoryDir) {
 ...
 */
export function createHeartbeatSummarizer(api, config, workspaceDir) {
    let intervalId = null;
    let isRunning = false;
 ...
 * 手動觸發摘要生成（供 RPC 調用）
 */
export async function triggerSummarization(config, workspaceDir) {
    const memoryDir = getMemoryDir(workspaceDir);
    const reportPath = getReportPath(workspaceDir);
    const logPath = getLatestSessionLog(memoryDir);
    if (!logPath) {
 ...
 * 讀取最新的日報
 */
export function getLatestReport(workspaceDir) {
    const reportPath = getReportPath(workspaceDir);
