...
import type { MemoryGuardianConfig, RagInjection, SearchResult } from '../types.js';
 ...
 * 格式化 RAG 結果為 XML
 */
function formatHistoricalMemoryXml(results: SearchResult[], query: string): string {
  if (results.length === 0) {
    return '';
  }
 ...
 */
export async function getRagInjectionContent(
  query: string,
  config: MemoryGuardianConfig
): Promise<string | null> {
 ...
 */
export async function createRagBootstrapFile(
  query: string,
  config: MemoryGuardianConfig
): Promise<{
  path: string;
 ...
 * 排除簡單的問候語和命令
 */
export function shouldPerformRag(query: string): boolean {
